...
async function generateAct(org, cp, invoice) {
  const templatePath = path.join(__dirname, 'act_template.xlsx');
  ...
  const title = `Акт оказанных услуг № ${invoice.number} от ${formatRuDate(invoice.date)} г.`;
  ...
  xml = xml.split('{{DIRECTOR_EXECUTOR}}').join(escXml(org.director || ''));
  xml = xml.split('{{DIRECTOR_CUSTOMER}}').join(escXml(cp?.name || ''));
  xml = xml.split('{{SIGNATURE_EXECUTOR}}').join(escXml('___________________________ /                     /'));
  xml = xml.split('{{SIGNATURE_CUSTOMER}}').join(escXml('___________________________ /                     /'));
  xml = xml.split('{{VAT_VALUE}}').join(escXml(invoice.vatType === 'none' ? '—' : fmt(invoice.vatAmount)));
